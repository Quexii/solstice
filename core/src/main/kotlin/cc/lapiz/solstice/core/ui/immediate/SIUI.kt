package cc.lapiz.solstice.core.ui.immediate
...
object SIUI {
	...
	fun label(text: String, align: TextAlign = TextAlign.LeftTop) {
		NVcanvas.text(cursorX, cursorY, text, NVcanvas.HEX(style.textColor), style.fontSize, style.font, align)
	...
	fun button(label: String, width: Float = 120f, height: Float = 30f): Boolean {
		...
		NVcanvas.roundedRect(x, y, width, height, style.buttonRounding, NVcanvas.HEX(color))

		NVcanvas.text(x + width / 2, y + height / 2, label, NVcanvas.HEX(style.buttonColorText), style.fontSize, style.font, TextAlign.CenterMiddle)
	...
	fun imageButton(image: SpriteResource, width: Float, height: Float, imageWidth: Float, imageHeight: Float): Boolean {
		val id = generateId()
		val x = cursorX
		val y = cursorY
	...
	fun checkbox(label: String, checked: Boolean): Boolean {
		...
		NVcanvas.roundedRect(x, y, size, size, 2f, NVcanvas.HEX(bgColor))
		...
		if (newChecked) {
			...
			NVcanvas.nvStrokeColor(NVcanvas.HEX(style.checkboxColorCheck))
			NVcanvas.nvStrokeWidth(2f)
			NVcanvas.nvStroke()
		}

		NVcanvas.text(x + size + 8f, y + size / 2, label, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftMiddle)
	...
	fun slider(label: String, value: Float, min: Float, max: Float, width: Float = 200f): Float {
		...
		NVcanvas.text(x, y + height / 2, "$label: %.2f".format(value), NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftMiddle)
		...
		NVcanvas.roundedRect(sliderX, trackY, width, 4f, 2f, NVcanvas.HEX(style.sliderColorTrack))

		val fillWidth = ((newValue - min) / (max - min)) * width
		NVcanvas.roundedRect(sliderX, trackY, fillWidth, 4f, 2f, NVcanvas.HEX(style.sliderColorFill))

		val handleX = sliderX + fillWidth
		NVcanvas.circle(handleX, y + height / 2, style.sliderHandleRadius, NVcanvas.HEX(style.sliderColorHandle))
	...
	fun textField(label: String, text: String, width: Float = 200f, height: Float = 30f): String {
		...
		NVcanvas.roundedRect(x, y, width, height, 4f, NVcanvas.HEX(style.textFieldColorBg))
		NVcanvas.nvBeginPath()
		NVcanvas.nvRoundedRect(x, y, width, height, 4f)
		NVcanvas.nvStrokeColor(NVcanvas.HEX(bgColor))
		NVcanvas.nvStrokeWidth(2f)
		NVcanvas.nvStroke()

		NVcanvas.text(x + 8f, y + height / 2, state.text.ifEmpty { label }, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftMiddle)
	...
	fun image(image: SpriteResource, width: Float, height: Float) {
		val x = cursorX
		val y = cursorY
	...
	fun collapsingHeader(label: String, id: String = label): Boolean {
		...
		val bgColor = if (isHovered) style.buttonColorHover else style.checkboxColorBg
		NVcanvas.roundedRect(x, y, width, height, 3f, NVcanvas.HEX(bgColor))
		...
		NVcanvas.nvFillColor(NVcanvas.HEX(style.textColor))
		NVcanvas.nvFill()

		NVcanvas.text(x + 20f, y + height / 2, label, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftMiddle)
	...
	fun beginWindow(title: String, x: Float, y: Float, width: Float, height: Float, header: Boolean = true) {
		NVcanvas.roundedRect(x, y, width, height, 8f, NVcanvas.HEX(0x1A202CFF))

		if (header) {
			NVcanvas.nvBeginPath()
			NVcanvas.nvRoundedRectVarying(x, y, width, 30f, 8f, 8f, 0f, 0f)
			NVcanvas.nvFillColor(NVcanvas.HEX(0x2D3748FF))
			NVcanvas.nvFill()

			NVcanvas.text(x + 12f, y + 15f, title, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftMiddle)
		}
	...
	fun labeledRow(label: String, vararg values: Pair<String, Any>) {
		...
		NVcanvas.text(cursorX, cursorY, label, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftTop)
		...
		for ((subLabel, value) in values) {
			NVcanvas.text(cursorX, cursorY, "$subLabel:", NVcanvas.HEX(0xA0AEC0FF), style.fontSize - 2f, style.font, TextAlign.LeftTop)
			...
			NVcanvas.text(cursorX, cursorY, valueText, NVcanvas.HEX(style.textColor), style.fontSize, style.font, TextAlign.LeftTop)
